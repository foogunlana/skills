...
Product manager skill for [Ralph](https://github.com/chrismdp/ralph) autonomous building. Manages beads (issues) through interactive conversation. This skill NEVER edits code, only manages beads.
...
1. **Start the watcher script** in background (run this first):
   ```bash
   pgrep -f "watch-blocked.sh" > /dev/null || (nohup ./watch-blocked.sh 10 "$(pwd)" > /tmp/ralph-watcher.log 2>&1 &)
   ```
...
To start manually:
```bash
./watch-blocked.sh 10 /path/to/project &
```
